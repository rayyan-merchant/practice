...
# 3. Encoding categorical features (Label Encoding, One-Hot Encoding)\# 4. Exploratory Data Analysis (EDA)
...
# Choose which DataFrame to proceed with (dropna/manual/imputed)
df = df_imputed.copy()

# -----------------------------
# 3. Encoding Categorical Variables
# -----------------------------
# Label Encoding for binary categorical variables or target label
label_enc = LabelEncoder()

# Example: encode target column 'target'
...
# Split the data
delimiter = 0.2  # 20% test size
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=delimeter, random_state=42, stratify=y
)

# Feature scaling: important for SVM
del scaler = StandardScaler()
...
cv_scores = cross_val_score(model_svm, X_train_scaled, y_train, cv=kf)
print(f"K-Fold CV ({k} folds) scores: {cv_scores}")
...
# -----------------------------
loo = LeaveOneOut()
loo_scores = cross_val_score(model_svm, X_train_scaled, y_train, cv=loo)
